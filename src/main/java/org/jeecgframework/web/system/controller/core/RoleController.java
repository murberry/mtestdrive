...
public class RoleController extends BaseController {
	...
	public ModelAndView userList(HttpServletRequest request) {
		...
		try {
			request.setAttribute("roleName", new String(roleName.getBytes("iso8859-1"), "utf-8"));
	...
	public void roleUserDatagrid(TSUser user,HttpServletRequest request, HttpServletResponse response, DataGrid dataGrid) {
        ...
        TSRoleUser ruser = null;
        for(int i=0; i<roleUser.size(); i++){
        	ruser = roleUser.get(i);
        	if(ruser != null && ruser.getTSUser() != null && (short)1 == ruser.getTSUser().getDeleteFlag()  ){
        		roleUser.remove(ruser);
        		i--;
        	}
        }
