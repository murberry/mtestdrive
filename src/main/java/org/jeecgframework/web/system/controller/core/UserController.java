...
public class UserController extends BaseController {
	...
	public String user(HttpServletRequest request) {
		...
		request.setAttribute("departsReplace", RoletoJson.listToReplaceStr(departList, "departname", "id"));
		departList.clear();
		request.setAttribute("curtUserKey", ResourceUtil.getSessionUserName().getUserKey());
...
//			}
	...
	public void datagrid(TSUser user,HttpServletRequest request, HttpServletResponse response, DataGrid dataGrid) {
        ...
        if(StringUtil.isNotEmpty(userName)){
        	cq.add(Restrictions.like("userName", '%'+userName+'%'));
        }
        String realName = user.getRealName();
        if(StringUtil.isNotEmpty(realName)){
        	cq.add(Restrictions.like("realName", '%'+realName+'%'));
        }
...
//            users.setTSDepart(user.getTSDepart());
			...
			DetachedCriteria dbUserDc = DetachedCriteria.forClass(TSUser.class);
			dbUserDc.add(Restrictions.eq("userName",user.getUserName()));
			dbUserDc.add(Restrictions.eq("deleteFlag",Globals.Delete_Normal));
			List<TSUser> dbUsers = systemService.findByDetached(dbUserDc);
			boolean isExist = false;
			if(!ListUtils.isNullOrEmpty(dbUsers)){
				TSUser users = dbUsers.get(0);
				if(users != null){
	...
	public ModelAndView addorupdate(TSUser user, HttpServletRequest req) {
		...
		req.setAttribute("roleName", ResourceUtil.getSessionUserName().getUserKey());
