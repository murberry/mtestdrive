...
public class SystemServiceImpl extends CommonServiceImpl implements SystemService {
	 ...
	 * 添加日志
	 */
	public void addSimpleLog(String logcontent, Short loglevel, Short operatetype) {
		TSLog log = new TSLog();
		log.setLogcontent(logcontent);
		log.setLoglevel(loglevel);
		log.setOperatetype(operatetype);
	 ...
	 * @return
	 */
	public List<TSType> getTypesByGroupId(String groupId) {
		
		return commonDao.findHql("from TSType where typegroupid = ?",groupId);
	...
	public String excuteProdurce(final String sql, final String obj1, final String obj2){
		String param2Value = (String) commonDao.getJdbcTemplate().execute(   
			     new CallableStatementCreator() {   
			        public CallableStatement createCallableStatement(Connection con) throws SQLException {   
			         //  String storedProc = "{call sp_select_table (?,?)}";// 调用的sql   
			        	  String storedProc = sql;// 调用的sql   
			           CallableStatement cs = con.prepareCall(storedProc);   
			           cs.setString(1, obj1);// 设置输入参数的值   
			           cs.setString(2, obj2);
	...
	public String excuteProdurce2(final String sql, final String obj1, final String obj2, final String obj3){
		String param2Value = (String) commonDao.getJdbcTemplate().execute(   
			     new CallableStatementCreator() {   
			        public CallableStatement createCallableStatement(Connection con) throws SQLException {   
			         //  String storedProc = "{call sp_select_table (?,?)}";// 调用的sql   
