...
//		fetchCustomerInfo();// 客户信息
	...
	public void fetchTestDrivePurchase() {
		...
		//远程取更新值
		String lastModifiedTime=querySdf.format(sfModified);
		String sql="select+Id,GPSExternalID__c,PurchaseDealer__c,CarPurchase__c,PurchaseTime__c,PurchaseModel__c,LastModifiedDate+from+GPSTestDrive__c+where+CarPurchase__c=true+and+LastModifiedDate%3E"+lastModifiedTime;
		logger.info("SF最后记录变更时间："+lastModifiedTime);
		...
		if (array != null) {
			...
			try {
				...
				logger.error("处理第"+processed+"/"+array.length()+"条购买记录时出错，本次处理退出：json="+obj);
				e.printStackTrace();
			}
		} else {
